...
import { EngravingContent, EngravingType } from "@/modules/products/types";
import { ImageIcon, QrCodeIcon, Type } from "lucide-react";

interface EngravingPreviewProps {
  content?: EngravingContent;
  type: EngravingType;
}

const getTypeIcon = (type: EngravingType) => {
  switch (type) {
    case "text":
...
const getTypeLabel = (type: EngravingType) => {
  switch (type) {
    case "text":
    ...
    case "qr_code":
      return "QR Code Engraving";
  }
};

export const EngravingPreview = ({ content, type }: EngravingPreviewProps) => {
